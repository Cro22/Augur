...
def _write_skewed_trace(path, n=120, seed=4):
    rng = np.random.default_rng(seed)
    recs = []
    for i in range(n):
...
def test_quantile_fit_report_predict(tmp_path, capsys):
    trace = tmp_path / "trace.jsonl"
    model = tmp_path / "model.json"
    _write_skewed_trace(trace)

    assert main(["fit", "--trace", str(trace), "--out", str(model),
                 "--dist", "quantile"]) == 0
    obj = json.loads(model.read_text())
    assert obj["dist"] == "quantile"
    assert obj["models"]["m"]["method"] == "quantile_reg"
    assert "quantiles" in obj["models"]["m"]

    assert main(["report", "--model", str(model)]) == 0
    out = capsys.readouterr().out
    assert "[quantile]" in out
    assert "p95 out~in" in out

    assert main(["predict", "--model", str(model), "--model-name", "m",
                 "--input-tokens", "1500", "--price-out", "10"]) == 0
    out = capsys.readouterr().out
    assert "p50" in out and "p95" in out


def test_emit_with_run_correlation_flag(tmp_path, capsys):
    trace = tmp_path / "trace.jsonl"
    model = tmp_path / "model.json"
    out = tmp_path / "pred.jsonl"
    _write_skewed_trace(trace)
    main(["fit", "--trace", str(trace), "--out", str(model), "--dist", "quantile"])
    capsys.readouterr()

    rc = main(["emit-trace", "--model", str(model), "--out", str(out),
               "--runs", "20", "--run-correlation", "0.7", "--seed", "3"])
    assert rc == 0
    recs = load_trace(str(out))
    assert len({r.run_id for r in recs}) == 20
